...
void pwm_setup()
{
	...
	timer_set_oc_mode(TIM4, TIM_OC4,
			  TIM_OCM_PWM2); // so it is in contra phase
...
int main(void)
{
	...
	timer_set_oc_value(
	    TIM4, TIM_OC4,
	    1024); // 0% duty for right motor (because it is not wired yet)
	...
	while (1) {
		...
		if (new_measure) {
			/* Obtain the difference between the former and new
			 * measure */
			...
			if (former_right_encoder >= new_right_encoder) {
				right_count =
				    former_right_encoder - new_right_encoder;
			} else {
