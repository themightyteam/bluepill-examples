...
void write_raw_data_example()
{
	...
	usart_send_string_stop_on_zero(
	...
	case RESULT_OK: /*everything ok*/
		usart_send_string_stop_on_zero(
		    (uint8_t *)"Verification of written data: ",
		    FLASH_STORAGE_SIZE);
		flash_read_data(FLASH_OPERATION_ADDRESS, FLASH_STORAGE_SIZE,
				str_verify);
		usart_send_string_stop_on_zero(str_verify, FLASH_STORAGE_SIZE);
		break;
	case FLASH_WRONG_DATA_WRITTEN: /*data read from Flash is different than
					  written data*/
		usart_send_string_stop_on_zero(
		    (uint8_t *)"Wrong data written into flash memory",
		    FLASH_STORAGE_SIZE);
		break;
	default: /*wrong flags' values in Flash Status Register (FLASH_SR)*/
		usart_send_string_stop_on_zero(
		    (uint8_t *)"Wrong value of FLASH_SR: ", FLASH_STORAGE_SIZE);
...
void write_structured_data_example()
{
	...
	usart_send_string_stop_on_zero(
	    (uint8_t *)"Loading some data to the struct:\n\r",
	    FLASH_STORAGE_SIZE);
...
void read_structured_data_example()
{
	...
	clean_persistent_structured_data(); // Clean RAM before loading from
					    // flash
	...
	printf("\n\n   myuint32 %u\n"
	       ...
	       get_persistant_mystring(), get_persistant_mychar(),
